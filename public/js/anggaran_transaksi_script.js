...
const upsertId = document.getElementById('upsert_id')
let dataId
    ...
    updateProductButton.addEventListener('click', function() {
        ...
        reqAjax('GET', '/get_anggaran_by_id?id=' + id, function(err, response) {
            response.forEach(res => {
                ...
                upsertId.value = res.upsert_id
            })
        })
        reqAjax('GET','/get_kategori_transaksi_by_jenis_transaksi_id_not_show/?id1=2&id2=3&id3=3&id4=4',function(err, response){
            setTimeout(() => {
                response.forEach(res => {
                    const option = document.createElement('option')
                    option.value = res.id
