...
class TransaksiController extends Controller
{
            ...
            if ($user->payment->status == 'pending') {
                if (Transaksi::where('user_id', auth()->user()->id)->count() > 50) {
                    ...
                    if (empty($data_anggaran[0]['jumlah'])) {
                        ...
                        // 0 untuk kebutuhan,
                        // 1 untuk keinginana
                        // 3 untuk jenis traonsaksi pengeluran pokok,
                        // 4 untuk jenis transaksi pegeluran tambahan
                        DatabaseHelper::updateAnggaran(0,3);
                        DatabaseHelper::updateAnggaran(1,4);
    ...
    public function api4()
    {
        ...
        $records = Transaksi::where('user_id', auth()->user()->id)
            ...
            ->groupBy(DB::raw('DATE(tanggal)'), 'transaksis.jenis_transaksi_id')
            ->get();
        ...
        foreach ($dataTanggal as $tanggal) {
            ...
            $jumlahData = $records->where('tanggal', $tanggalData)->whereIn('jenis_transaksi_id', [request()->id, request()->id2]);;
