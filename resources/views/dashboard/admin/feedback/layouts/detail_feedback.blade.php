...
@section('container')
    <section class="mb-32">
        <h1 class="mt-2 mb-3 text-4xl font-semibold">{{ $data[0]->kategori }} <span class="text-sm text-gray-500 font-normal">
                #{{ $data[0]->no_feedback }} </span></h1>
        <div class="mb-7 flex items-center gap-2">
            ...
            <p class="text-sm text-gray-500"><span
                    class="font-semibold">{{ $data[0]->progres_dev_by->username ?? '--' }}</span> <span
                    class="{{ $data[0]->progres_by == null ? 'hidden' : '' }}"><span>
                        @switch($data[0]->progres)
                            @case('on going')
                                sedang mengerjakan
                            @break
        ...
        <div class="grid grid-cols-5 gap-3">
            ...
            <div class="bg-white p-5 rounded-md shadow-md h-fit">
                ...
                <p class="text-gray-700 text-sm font-medium mt-1">
                    {{ $data[0]->progres == 'done' ? $data[0]->progres_dev_by->username : '--' ?? '--' }}</p>
