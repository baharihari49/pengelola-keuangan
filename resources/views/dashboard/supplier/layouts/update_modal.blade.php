            ...
            <form method="POST" action="/supplier_or_customer_update">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2 grid grid-cols-2 gap-4">
                        <div class="">
                            <label for="tipe-supplier-update" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tipe</label>
                            <select oninput="disabledBtn(this)" id="tipe-supplier-update" name="jenis_transaksi_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                        ...
                        <div class="">
                            <label for="jenis_transaksi-update" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jenis Transaksi</label>
                            <select oninput="disabledBtn(this)" id="jenis_transaksi-update" name="jenis_transaksi_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                <option>Select Jenis transaksi</option>
                                <option id="jnTransaksiSup" value="1">Pendapatan Tetap</option>
                                <option id="jnTransaksiSup" value="2">Pendapatan Tidak Tetap</option>
                                <option id="jnTransaksiSup" value="3">Pengeluaran Pokok</option>
                                <option id="jnTransaksiSup" value="4">Pengeluaran Tambahan</option>
                            </select>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // const tipeSupllierUpdate = document.getElementById('tipe-supplier-update')

    // const getJenisTransaksi = async() => {
    //     try {
    //         const response = await fetch(`/get-jenis-transaksi-by-tipe-supplier/?id=${tipeSupllierUpdate.value}`);
    //         if (!response.ok) {
    ...
    //         const jenisTransaksiUpdate = document.getElementById('jenis_transaksi-update')
    //         jenisTransaksiUpdate.innerHTML = '<option selected="">Select Jenis transaksi</option>'
    ...
    //             jenisTransaksiUpdate.appendChild(option);
    //         });
    ...
    //     tipeSupllierUpdate.addEventListener('input', async function () {
    //     try {
    //         const response = await fetch(`/get-jenis-transaksi-by-tipe-supplier/?id=${tipeSupllierUpdate.value}`);
    //         if (!response.ok) {
    //             throw new Error(`HTTP error! Status: ${response.status}`);
