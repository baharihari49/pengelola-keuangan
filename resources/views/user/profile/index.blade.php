...
<div class="bg-white rounded-xl p-6 shadow-md h-fit mb-36">
    ...
    <form action="/store_info_bisnis" method="POST" enctype="multipart/form-data">
        ...
        <div id="deleteLogoModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                <!-- Modal content -->
                <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                    <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteLogoModal">
                    ...
                    <div class="flex justify-center items-center gap-4">
                        <form action="">
                            <button data-modal-toggle="deleteLogoModal" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                No, cancel
                            </button>
                        </form>
                        <form id="" method="POST" action="/delete_logo">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="logo" value="{{ isset($info_bisnis[0]) ? $info_bisnis[0]->logo : ''}}">
        ...
        <div class="grid md:grid-cols-2 md:gap-6 items-end">
            <div class="mb-5 lg:mb-0">
                <img id="avatar" class="w-20 h-20 rounded mb-2" src="{{ isset($info_bisnis[0]->logo) ? 'storage/' . $info_bisnis[0]->logo : 'image/logo/placeholder_logo.png' }}"
                        alt="Logo">
                <button data-modal-toggle="deleteLogoModal" class="py-1 {{isset($info_bisnis[0]->logo) ? '' : 'hidden '}} px-2 text-white font-medium mb-3 bg-red-700 text-xs rounded-md">hapus logo</button>
